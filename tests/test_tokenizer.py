...
def test_tokenizer_from_pretrained() -> None:
    """测试从预训练模型加载分词器"""
    print("=== 测试从预训练模型加载分词器 ===")

    try:
        # 这里只是测试导入，实际使用时需要有效的模型路径
        HF2Tokenizer.from_pretrained("gpt2")
...
def test_tokenizer_training() -> None:
    """测试分词器训练功能"""
    print("=== 测试分词器训练功能 ===")
    ...
    try:
        ...
        # 测试解码功能
        decoded = tokenizer.decode(cast(list[int], encoded))
...
def test_tokenizer_encoding() -> None:
    ...
    # 测试带前缀和后缀的编码
    encoded_with_special = tokenizer.encode(
        text, prepend="<|bos|>", append="<|assistant_end|>"
    )
    print(f"带特殊令牌的编码: {encoded_with_special}")

    # 测试ID到令牌转换
    if encoded:
        first_token_id = cast(int, encoded[0])
        token_str = tokenizer.id_to_token(first_token_id)
        print(f"ID {first_token_id} 对应的令牌: '{token_str}'")

    print("✓ 分词器编码功能测试通过\n")


def test_tokenizer_call_method() -> None:
    """测试分词器的__call__方法"""
