...
class TestLlamaBlock(unittest.TestCase):
    ...
    def test_forward_with_attention_mask(self) -> None:
        ...
        with torch.no_grad():
            output, _ = self.block(x, cos, sin, attention_mask)
    ...
    def test_forward_with_kv_cache(self) -> None:
        ...
        # 第一次前向传播获取缓存
        with torch.no_grad():
            _, block_kv_cache = self.block(x, cos, sin)

        # 第二次前向传播使用缓存
        x_new = torch.randn(batch_size, 5, hidden_dim)
        position_ids_new = (
            torch.arange(seq_len, seq_len + 5).unsqueeze(0).expand(batch_size, -1)
        )
        cos_new, sin_new = rotary(position_ids_new)

        with torch.no_grad():
            output2, block_kv_cache = self.block(
                x_new, cos_new, sin_new, block_kv_cache=block_kv_cache
            )
...
class TestLlamaTransformer(unittest.TestCase):
    ...
    def test_forward_pass_with_tokens(self) -> None:
        """测试使用token输入的前向传播"""
        ...
        print(
            f"每层KV缓存结构: key={kv_cache[0]['key'].shape}, value={kv_cache[0]['value'].shape}"
        )
        self.assertEqual(len(kv_cache), self.cfg.n_layers)
        print("✓ LlamaTransformer token输入前向传播测试通过")

    def test_forward_pass_with_embeddings(self) -> None:
    ...
    def test_generate_method(self) -> None:
        ...
        print(
            f"前{seq_len}个token与输入相同: {torch.equal(generated[:, :seq_len], inputs)}"
        )
        self.assertTrue(torch.equal(generated[:, :seq_len], inputs))
        print("✓ LlamaTransformer generate方法测试通过")

    def test_generate_with_single_sequence(self) -> None:
    ...
    def test_weight_tying(self) -> None:
        ...
        if self.cfg.lm_tie_weights:
            ...
            self.assertIsNot(
                model_no_tie.head.weight, model_no_tie.token_embedding.weight
            )

    def test_kv_cache_inference(self) -> None:
        """测试KV缓存推理"""
        batch_size = 1
        seq_len = 5
        ...
        with torch.no_grad():
            ...
            logits2, kv_cache = self.model(
                new_token, kv_cache=kv_cache, start_pos=seq_len
            )
    ...
    def test_from_pretrained(self) -> None:
        ...
        cfg = LLMConfig(lm_use_tokens=True, lm_tie_weights=True)
        ...
        try:
            ...
            print(f"[skip] Failed to load pretrained model: {e!s}")
            raise unittest.SkipTest(f"Failed to load pretrained model: {e!s}")

    def test_from_pretrained_with_custom_vocab(self) -> None:
        ...
        try:
            ...
            print(f"Skipping from_pretrained_with_custom_vocab test: {e!s}")
            raise unittest.SkipTest(f"Failed to load pretrained model: {e!s}")
