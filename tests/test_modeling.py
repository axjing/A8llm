...
def test_gpt_model() -> None:
    """测试完整GPT模型"""
    print("=== 测试完整GPT模型 ===")

    # 创建小型配置
    config = LLMConfig(vocab_size=100, n_embd=64, n_layers=2, n_heads=2, n_positions=32)
...
def test_optimizer() -> None:
    ...
    optimizer = model.configure_optimizers(
        weight_decay=0.01, learning_rate=0.001, betas=(0.9, 0.999), device_type="cpu"
    )
...
def test_gpt_estimate_mfu() -> None:
    """测试MFU估算"""
    print("=== 测试MFU估算 ===")
...
def test_gpt_from_pretrained_initialization() -> None:
    ...
    for model_type in supported_models:
        try:
            ...
            config_args: dict[str, Any] = {
                "gpt2": {"n_layers": 12, "n_heads": 12, "n_embd": 768},
                "gpt2-medium": {"n_layers": 24, "n_heads": 16, "n_embd": 1024},
                "gpt2-large": {"n_layers": 36, "n_heads": 20, "n_embd": 1280},
                "gpt2-xl": {"n_layers": 48, "n_heads": 25, "n_embd": 1600},
            }[model_type]
...
def test_gpt_from_pretrained_override_args() -> None:
    ...
    # 模拟配置创建
    config_args: dict[str, Any] = {"n_layers": 12, "n_heads": 12, "n_embd": 768}
    config_args["vocab_size"] = 50257
    config_args["n_positions"] = 1024
    config_args["bias"] = True
...
def test_gpt_from_pretrained_state_dict_keys() -> None:
...
def test_gpt_from_pretrained_integration() -> None:
...
def main() -> None:
    ...
    except (AssertionError, RuntimeError, ValueError, KeyError) as e:
        print(f"❌ 测试失败: {e}")
        import traceback
