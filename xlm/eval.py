...
from xlm.common.file_os import download_file_with_lock, get_base_dir
from xlm.common.logger import print0
from xlm.common.tokenizer import HF2Tokenizer, get_token_bytes
from xlm.data.text_pretrain_loader import tokenizing_distributed_data_loader_bos_bestfit
from xlm.engine.engine_inference import Engine
from xlm.engine.utils_checkpoints import load_model
from xlm.evaluator.eval_core import Tokenizer, evaluate_task
from xlm.evaluator.eval_loss import evaluate_bpb
from xlm.report import get_report
from xlm.trainer.distributed import (
...
class ModelWrapper:
    ...
    def __init__(self, model: Any, max_seq_len: int | None = None) -> None:
        # model: a transformers model object (untyped third-party stub)
        self.model = model
    ...
    def __call__(
        ...
        logits: torch.Tensor = self.model(input_ids).logits
        if targets is None:
            return logits
...
def load_hf_model(
    hf_path: str, device: torch.device
) -> tuple[ModelWrapper, HF2Tokenizer]:
    """Load a HuggingFace model and tokenizer."""
    print0(f"Loading HuggingFace model from: {hf_path}")
    model: Any = AutoModelForCausalLM.from_pretrained(hf_path)
    model.to(device)
    model.eval()
...
def get_hf_token_bytes(tokenizer: HF2Tokenizer, device: str = "cpu") -> torch.Tensor:
    """Compute token_bytes tensor for a HuggingFace tokenizer."""
    vocab_size = tokenizer.tokenizer.get_vocab_size()
    token_bytes: torch.Tensor = torch.zeros(
        vocab_size, dtype=torch.int64, device=device
    )
...
def main() -> None:
    ...
    eval_modes: set[str] = {mode.strip() for mode in args.eval.split(",")}
    valid_modes: set[str] = {"core", "bpb", "sample"}
    ...
    if is_hf_model:
        ...
        sequence_len: int = model.max_seq_len or 1024
        token_bytes = get_hf_token_bytes(tokenizer, device=device.type)
    ...
    if "sample" in eval_modes and not is_hf_model:
        ...
        if ddp_rank == 0:
            ...
            tokens = cast(list[int], tokenizer("", prepend="<|bos|>"))
            uncond, _ = engine.generate_batch(
                tokens, num_samples=8, max_tokens=128, temperature=1.0
            )
            for uncond_sample in uncond:
                sample_str = tokenizer.decode(uncond_sample)
                print0("-" * 80)
                print0(sample_str)
