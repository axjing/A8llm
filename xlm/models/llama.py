...
from xlm.models.layers import GroupedQueryAttention, LlamaMLP, RMSNorm
from xlm.models.position_embedding import RotaryEmbedding
from xlm.models.transformer_utils import (
    build_muon_optimizer,
    count_scaling_params,
    estimate_flops,
    init_weights,
)

if TYPE_CHECKING:
    from xlm.trainer.optim import MuonAdamW
...
class LlamaBlock(nn.Module):
    ...
    def forward(
        ...
        block_kv_cache: dict[str, torch.Tensor] | None = None,
...
class LlamaTransformer(nn.Module):
    ...
    def _init_weights(self, module: nn.Module) -> None:
        ...
        init_weights(module, rms_norm=True)
    ...
    def forward(
        ...
        if self.lm_use_tokens:
            ...
            if x.dtype in (torch.float16, torch.float32, torch.float64):
                ...
                print(
                    "Input is already embedded (float type), skipping token_embedding"
                )
                ...
                raise ValueError(
                    f"Unsupported input dtype: {x.dtype}. Expected integer (token indices) or float (embeddings)."
                )

        bsz, seq_len_crr, _ = x.size()

        # Create position_ids for the current sequence based on start_pos
        current_position_ids = (
            torch.arange(start_pos, start_pos + seq_len_crr, device=x.device)
            .unsqueeze(0)
            .expand(bsz, -1)
        )
        ...
        if kv_cache is None:
            kv_cache = [cast(dict[str, torch.Tensor] | None, None)] * len(self.blocks)
    ...
    def generate(self, inputs: torch.Tensor, max_new_tokens: int = 20) -> torch.Tensor:
        """
        ...
        for i in range(max_new_tokens):
            ...
            decode_step_outputs, kv_cache_list = self.forward(
                next_output,
                attention_mask=None,
                kv_cache=kv_cache_list,
                start_pos=current_tokens_start_pos,
            )
    ...
    # ------------------------------------------------------------------
    # Shared utilities (delegated to transformer_utils)
    ...
    def num_scaling_params(self) -> dict[str, int]:
        ...
        return count_scaling_params(
            ...
            n_intermediate=(
                cfg.n_intermediate if cfg.n_intermediate is not None else 4 * cfg.n_embd
            ),
    ...
    @classmethod
    def from_pretrained(cls, cfg: LLMConfig) -> "LlamaTransformer":
        import json

        import safetensors
        from huggingface_hub import hf_hub_download

        # The huggingface_hub type stubs do not re-export this error from utils.
        ...
        if hasattr(cfg, "vocab_size"):
            if cfg.vocab_size < original_vocab_size:
                raise ValueError(
                    f"Config vocab size ({cfg.vocab_size}) is smaller than pretrained model vocab size ({original_vocab_size})"
                )
        ...
        cfg.rotary_emb_base = getattr(
            hf_config, "rope_theta", getattr(hf_config, "rope_scaling", 10000.0)
        )
        ...
        try:
            ...
            safetensors_filenames = sorted(set(index["weight_map"].values()))
