...
from xlm.models.modality_projector import ModalityProjector
from xlm.models.utils import top_k_top_p_filtering
from xlm.models.vision_transformer import ViT


class VisionLanguageModel(nn.Module):
    def __init__(self, cfg: VLMConfig, load_backbone: bool = True) -> None:
        super().__init__()
    ...
    def _replace_img_tokens_with_embd(
        ...
        updata_token_embd[mask] = image_embd.view(-1, image_embd.size(-1)).to(
            updata_token_embd.dtype
        )
    ...
    def _process_images(
        self,
        images: list[torch.Tensor] | list[list[torch.Tensor]] | torch.Tensor,
        device: torch.device,
    ) -> torch.Tensor | None:
        if isinstance(images, list):
            ...
            if not images:  # Handle cases with no images
                ...
                return torch.cat(cast(list[torch.Tensor], images), dim=0).to(device)
    ...
    def generate(
        ...
        for _ in range(max_new_tokens):
            if greedy:
                ...
                fitered_logits = top_k_top_p_filtering(
            ...
            next_token_embd = self.decoder.token_embedding(
                next_token_id
            )  # [bsz,1,n_embd]
