...
class ViTPatchEmbeddings(nn.Module):
    # Ref:[SigLIP](https://github.com/huggingface/transformers/blob/main/xlm/transformers/models/siglip/modeling_siglip.py#L245)
    def __init__(self, cfg: VLMConfig) -> None:
        ...
        if self.cls_flag:
            ...
            self.position_embedding = nn.Parameter(
                torch.rand(1, self.n_patch + 1, self.n_embd)
            )
        else:
            self.position_embedding = nn.Parameter(
                torch.rand(1, self.n_patch, self.n_embd)
            )
...
class ViTMultiHeadAttention(nn.Module):
    # https://github.com/huggingface/transformers/blob/main/xlm/transformers/models/siglip/modeling_siglip.py#L381
    ...
    def __init__(self, cfg: VLMConfig) -> None:
        ...
        assert self.n_embd % self.n_head == 0, (
            f"embd dim must be divisible by n_head,current:n_embed % n_head={self.n_embd}%{self.n_head}"
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.sdpa:
            # torch's Module.__call__ stubs as Callable[..., Any]; pin the
            # local type so the projection chain below stays a Tensor.
            y: torch.Tensor = torch.nn.functional.scaled_dot_product_attention(
                q,
                k,
                v,
                attn_mask=None,
                dropout_p=self.droput if self.training else 0.0,
            ...
            y = (
                attn @ v
            )  # (bsz,n_head,seq_len,seq_len) @ (bsz,n_head,seq_len,n_embed) -> (bsz,n_head,seq_len,head_dim)
...
class ViTMLP(nn.Module):
    # https://github.com/huggingface/transformers/blob/main/xlm/transformers/models/siglip/modeling_siglip.py#L453
    def __init__(self, cfg: VLMConfig) -> None:
        super().__init__()
...
class ViT(nn.Module):
    def __init__(self, cfg: VLMConfig) -> None:
        ...
        self.blocks = nn.ModuleList(
            [ViTBlock(self.cfg) for _ in range(self.cfg.vit_n_layers)]
        )
        self.layer_norm = nn.LayerNorm(
            self.cfg.vit_n_embd, eps=self.cfg.vit_layernorm_eps
        )
    ...
    @classmethod
    def from_pretrained(cls, cfg: VLMConfig) -> "ViT":
        import safetensors
        from huggingface_hub import hf_hub_download
        from transformers import Siglip2VisionConfig

        hf_config = Siglip2VisionConfig.from_pretrained(
            cfg.vit_model_type, filename="model.safetensors"
        )
        ...
        # ViT layer norms use this eps; the old name wrote a phantom attribute
        # that no code ever read, so the loaded model kept the default eps.
        cfg.vit_layernorm_eps = hf_config.layer_norm_eps
        cfg.n_heads = hf_config.num_attention_heads
        cfg.n_layers = hf_config.num_hidden_layers
        assert isinstance(hf_config.patch_size, int), (
            "SigLIP patch_size must be a single int"
        )
        ...
        with safetensors.safe_open(
            ...
            for hf_key, our_key in mapping.items():
                if hf_key in keys and our_key in sd:
