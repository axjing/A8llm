...
from xlm.common.tokenizer import HF2Tokenizer, RustBPETokenizer
...
def render_prompts_mc(
    ...
    fewshot_examples: list[TaskItem] | None = None,
) -> list[str]:
...
def batch_sequences_mc(
    tokenizer: Tokenizer, prompts: list[str]
) -> tuple[list[list[int]], list[int], list[int]]:
    # In multiple choice, contexts are the same but the continuation is different (common prefix)
    tokens = cast(
        list[list[int]],
        tokenizer(prompts, prepend=tokenizer.get_bos_token_id()),
    )
...
def evaluate_example(
    ...
    fewshot_examples: list[TaskItem] = []
    if num_fewshot > 0:
