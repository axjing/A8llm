...
torchrun --nproc_per_node=8 -m xlm.train_llm

If you are only on CPU/Macbook, you'll want to train a much much smaller LLM. Example:
python -m xlm.train_llm --depth=4 --max-seq-len=512 --device-batch-size=1 --eval-tokens=512 --core-metric-every=-1 --total-batch-size=512 --num-iterations=20
"""
...
from xlm.common.logger import init_swanlab, print0, print_banner
from xlm.common.tokenizer import get_token_bytes, get_tokenizer
from xlm.data.text_pretrain_loader import (
    tokenizing_distributed_data_loader_bos_bestfit,
    tokenizing_distributed_data_loader_with_state_bos_bestfit,
)
from xlm.engine.engine_inference import Engine
from xlm.engine.utils_checkpoints import load_checkpoint, save_checkpoint
from xlm.eval import CoreResults, evaluate_core
from xlm.evaluator.eval_loss import evaluate_bpb
from xlm.models.config import LLMConfig
from xlm.models.flash_attention import HAS_FA3, USE_FA3
from xlm.models.gpt import GPT
from xlm.models.layers import Linear
...
from xlm.trainer.train_fp8 import (
    Float8LinearConfig,
...
def run_core_evaluation(
    ...
    centered_metrics = {
        f"core/{label}": value for label, value in results["centered_results"].items()
    }
    swanlab_run.log(
        {
            "total_training_flops": flops_so_far,
...
def train(args: TrainConfig) -> None:
    ...
    swanlab_run = init_swanlab(
        ...
        run_name=args.run,
        config=user_config,
        enabled=args.run != "dummy" and master_process,
    )
            ...
            swanlab_run.log(
                {
                    ...
                    "train/mfu": mfu,
                    "train/epoch": int(dataloader_state_dict["epoch"]),
                },
                step=step,
            )
